...
/*#1. add a songs table (mostly got it)*/
CREATE TABLE songs(
	id INT NOT NULL AUTO_INCREMENT,
	name VARCHAR(255) NOT NULL,
...
/*#2. change the name of the column name from bands to Band Name (got in theory couldnt code it) */
SELECT name AS 'Band Name'
FROM bands;

/*#3. SELECT the oldest album from the database(kinda got it) */
SELECT * FROM albums 
WHERE release_year IS NOT NULL
ORDER BY release_year asc /*use desc for newest album*/
LIMIT 1;

/* #4. Return all the bands that have an album (BOMBED)*/
SELECT DISTINCT bands.name AS 'Band Name'
FROM  bands
INNER JOIN albums ON bands.id = albums.band_id;

/* #5. Get all the bands with no albums in the database (got it in full)*/
SELECT distinct bands.name AS 'Band Name'
from bands
LEFT JOIN albums on bands.id = albums.band_id
WHERE albums.band_id IS NULL;
/*ANOTHER WAY 
AFTER LEFT JOIN NOT USING WHERE
GROUP BY album.band_id
HAVING COUNT(albums.id) = 0*/
 
 /* #6. get the longest album */ /*(STRUGGLE A BIT)*/
...
SELECT COUNT(name) as 'Number of Songs' FROM SONGS;

/*#7 update the release year that has no year on it (kinda easy)*/
SELECT * FROM albums
...
/*#8 insert a record of your favorite band and album (needed lil help)*/
SELECT * FROM bands;
INSERT INTO bands(id,name) VALUES(8,'Migos');
/*ANOTHER WAY TO DO IT*/
INSERT INTO bands (name) VALUES ('Migos');
SELECT * FROM albums;
INSERT INTO albums(id,name,release_year,band_id) VALUES(19,'Culture',2017,8);
/*ANOTHER WAY TO DO IT*/
INSERT INTO albums (name,release_year,band_id) VALUES('Culture',2017,8);

/*#9 delete a record of your favorite band and album (easy)*/
DELETE /*albums (not needed but still works) */ FROM albums WHERE band_id = 8;
DELETE /*bands (not needed but still works) */ FROM bands WHERE id = 8;

/* #10 get the average length from the songs (easy) */
SELECT AVG(length) as 'Average Song Duration'from songs;

/*#11 select the longest song from each album (kinda got)*/
SELECT albums.name AS 'Album', albums.release_year AS 'Release Year', MAX(songs.length) AS 'Duration' from albums
JOIN songs ON albums.id = songs.album_id
GROUP BY songs.album_id;

/*#12 Get the number of songs from each bands (struggle with double join) */
SELECT bands.name AS 'Bands', COUNT(songs.id) AS 'Number of songs' from bands
